...
pscores.bart <- function(treat=Z, outs=Y, covs=X, pscore=NULL){
#
N <- nrow(covs)
nouts <- 1 
ncovs <- ncol(covs)
...
# 	mod <- glm(dat,fam=binomial)
if(is.null(pscore)) {
	qx <- colMeans(pbart(y.train=treat, x.train=covs 
...
matching.wr.x3 <- function(z, score){
	...
	## cnts = number of times the jth untreated element enters the matched sample
	## ind.mt[untreated element used, which use of it] = which treated matched to
	## ind = array length nt + matched untreated, indices in dataset
