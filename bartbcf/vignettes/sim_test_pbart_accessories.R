...
bart2a_bcf <- mc.pbart_bcf(x.train=xt2,   y.train=YA[sort_order],  numtreated = nt, mc.cores=mc.cores , printevery=1000L, ndpost=2400, nskip=400, use_cauchy=TRUE, sparse=TRUE)
bart2b_bcf <- mc.pbart_bcf(x.train=xt2,   y.train=YB[sort_order],  numtreated = nt, mc.cores=mc.cores , printevery=1000L, ndpost=2400, nskip=400, use_cauchy=TRUE, sparse=TRUE)
bart2c_bcf <- mc.pbart_bcf(x.train=xt2,   y.train=YC[sort_order],  numtreated = nt, mc.cores=mc.cores , printevery=1000L, ndpost=2400, nskip=400, use_cauchy=TRUE, sparse=TRUE)




# bart2a <- mc.pbart(x.train=xt,   y.train=YA,  x.test=xp, ndpost=2400, nskip=400, mc.cores=mc.cores, sparse=TRUE)
# bart2b <- mc.pbart(x.train=xt,   y.train=YB,  x.test=xp, ndpost=2400, nskip=400, mc.cores=mc.cores, sparse=TRUE)
# bart2c <- mc.pbart(x.train=xt,   y.train=YC,  x.test=xp, ndpost=2400, nskip=400, mc.cores=mc.cores, sparse=TRUE)
...
results.store.a <- results.store[-1,] 
# print(results.store)
save(file="pbart_answers_b.Rdata", results.store.a)
library('dplyr')
results.store.a %>% group_by(surface, te_type, target) %>% summarize(avg_ef = mean(te, na.rm=TRUE)) %>% print(n=100)
results.store.a %>% group_by(surface, te_type, target) %>% summarize(avg_cil = mean(te_cil, na.rm=TRUE)) %>% print(n=100)
results.store.a %>% group_by(surface, te_type, target) %>% summarize(avg_cov = mean(covered, na.rm=TRUE)) %>% print(n=100)
## coverage
