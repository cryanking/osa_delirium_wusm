...
qx0=glm(data=obs[,c("treat",covs.cont.n,covs.cat.n)],formula=form.qx,family=binomial)$fitted
...
bart2a_bcf <- mc.wbart_bcf(x.train=xt2,   y.train=YA[sort_order],  numtreated = nt, mc.cores=mc.cores , printevery=1000L, ndpost=1600, nskip=200)
bart2b_bcf <- mc.wbart_bcf(x.train=xt2,   y.train=YB[sort_order],  numtreated = nt, mc.cores=mc.cores , printevery=1000L, ndpost=1600, nskip=200)
bart2c_bcf <- mc.wbart_bcf(x.train=xt2,   y.train=YC[sort_order],  numtreated = nt, mc.cores=mc.cores , printevery=1000L, ndpost=1600, nskip=200)


if(TRUE) {
bart2a <- mc.wbart(x.train=xt,   y.train=YA,  x.test=xp, mc.cores=mc.cores , printevery=1000L )
bart2b <- mc.wbart(x.train=xt,   y.train=YB,  x.test=xp, mc.cores=mc.cores , printevery=1000L )
bart2c <- mc.wbart(x.train=xt,   y.train=YC,  x.test=xp, mc.cores=mc.cores , printevery=1000L )
...
## point estimate bias
results.a.2[i,1]=4-bart2a_bcf$ate_est
# ci=bart2a_bcf$ate_est + c(-1,1)*1.96*bart2a_bcf$ate_sd
ci=bart2a_bcf$ate_ci
## includes truth
...
tmp2=apply(bart2a_bcf$yhat.train.treated-bart2a_bcf$yhat.train,2,mean)
precision.a.2[i,1] = sqrt(mean((tmp2-tauAis)^2))
rm(tmp2)

## targets are defined only among the treated
# b
results.b.2[i,1]=4-bart2b_bcf$ate_est
# ci=bart2b_bcf$ate_est + c(-1,1)*1.96*bart2b_bcf$ate_sd
ci=bart2b_bcf$ate_ci

results.b.2[i,2]=(ci[1]<4 & ci[2]>4)*1
results.b.2[i,3]=ci[2]-ci[1]
results.b.2[i,15]=(ci[1]<0 & ci[2]>0)*1

tmp2=apply(bart2b_bcf$yhat.train.treated[,seq.int(nt)]-bart2b_bcf$yhat.train[,seq.int(nt)],2,mean)
precision.b.2[i,1] = sqrt(mean((tmp2 - tauBis)^2))

# c
results.c.2[i,1]=4-bart2c_bcf$ate_est
# ci=bart2c_bcf$ate_est + c(-1,1)*1.96*bart2c_bcf$ate_sd
ci=bart2c_bcf$ate_ci
results.c.2[i,2]=(ci[1]<4 & ci[2]>4)*1
results.c.2[i,3]=ci[2]-ci[1]
results.c.2[i,15]=(ci[1]<0 & ci[2]>0)*1

tmp2=apply(bart2c_bcf$yhat.train.treated[,seq.int(nt)]-bart2c_bcf$yhat.train[,seq.int(nt)],2,mean)
...
results.a.2[i,4]=4-bart2a_bcf$att_est
ci=bart2a_bcf$att_est + c(-1,1)*1.96*bart2a_bcf$att_sd
ci=bart2a_bcf$att_ci

results.a.2[i,5]=(ci[1]<4 & ci[2]>4)*1
results.a.2[i,6]=ci[2]-ci[1]
results.a.2[i,16]=(ci[1]<0 & ci[2]>0)*1
tmp2=apply(bart2a_bcf$yhat.train.treated[,seq.int(nt)]-bart2a_bcf$yhat.train[,seq.int(nt)],2,mean)
precision.a.2[i,2] = sqrt(mean((tmp2-tauAis)^2))


# b
results.b.2[i,4]=4-bart2b_bcf$att_est
ci=bart2b_bcf$att_est + c(-1,1)*1.96*bart2b_bcf$att_sd
ci=bart2b_bcf$att_ci
...
results.c.2[i,4]=4-bart2c_bcf$att_est
ci=bart2c_bcf$att_est + c(-1,1)*1.96*bart2c_bcf$att_sd
ci=bart2c_bcf$att_ci
...
###############
## the LATE group doesn't exist in this sample (or is extremely small)
###############

## late fails as set up in this example because the pscores are never that high
if(FALSE) {
late_set <- which( qx_main[sort_order] < 0.6  &&  qx_main[sort_order] > 0.4) 

results.a.2[i,10]=4-bart2a_bcf$late_est
ci=bart2a_bcf$late_est + c(-1,1)*1.96*bart2a_bcf$late_sd
ci=bart2a_bcf$late_ci

results.a.2[i,11]=(ci[1]<4 & ci[2]>4)*1
results.a.2[i,12]=ci[2]-ci[1]
results.a.2[i,18]=(ci[1]<0 & ci[2]>0)*1
tmp2=apply(bart2a_bcf$yhat.train.treated[,late_set]-bart2a_bcf$yhat.train[,late_set],2,mean)
precision.a.2[i,4] = sqrt(mean((tmp2-tauAis)^2))


# b
results.b.2[i,10]=4-bart2b_bcf$late_est
ci=bart2b_bcf$late_est + c(-1,1)*1.96*bart2b_bcf$late_sd
ci=bart2b_bcf$late_ci

results.b.2[i,11]=(ci[1]<4 & ci[2]>4)*1
results.b.2[i,12]=ci[2]-ci[1]
results.b.2[i,18]=(ci[1]<0 & ci[2]>0)*1

tmp2=apply(bart2b_bcf$yhat.train.treated[,late_set]-bart2b_bcf$yhat.train[,late_set],2,mean)
precision.b.2[i,4] = sqrt(mean((tmp2-tauBis[resort_late])^2))
rm(tmp,tmp2)

# c

results.c.2[i,10]=4-bart2c_bcf$late_est
ci=bart2c_bcf$late_est + c(-1,1)*1.96*bart2c_bcf$late_sd
ci=bart2c_bcf$late_ci

results.c.2[i,11]=(ci[1]<4 & ci[2]>4)*1
results.c.2[i,12]=ci[2]-ci[1]
results.c.2[i,18]=(ci[1]<0 & ci[2]>0)*1

tmp2=apply(bart2c_bcf$yhat.train.treated[,late_set]-bart2c_bcf$yhat.train[,late_set],2,mean)
precision.c.2[i,4] = sqrt(mean((tmp2-tauCLs[resort_late])^2))
}

######################
## do the modified att instead
######################

results.a.2[i,10]=4-bart2a_bcf$att_modified_est
ci=bart2a_bcf$att_modified_est + c(-1,1)*1.96*bart2a_bcf$att_modified_sd
ci=bart2a_bcf$att_modified_ci
...
# b
results.b.2[i,10]=4-bart2b_bcf$att_modified_est
ci=bart2b_bcf$att_modified_est + c(-1,1)*1.96*bart2b_bcf$att_modified_sd
ci=bart2b_bcf$att_modified_ci
...
results.c.2[i,10]=4-bart2c_bcf$att_modified_est
ci=bart2c_bcf$att_modified_est + c(-1,1)*1.96*bart2c_bcf$att_modified_sd
ci=bart2c_bcf$att_modified_ci
...
tmp2=apply(bart2c_bcf$yhat.train.treated[,seq.int(nt)]-bart2c_bcf$yhat.train[,seq.int(nt)],2,mean)
precision.c.2[i,4] = sqrt(mean((tmp2-tauCis)^2))
...
save(file="bcf_test.Rdata", results.a.2, results.b.2, results.c.2, precision.a.2, precision.b.2, precision.c.2, results.a, results.b, results.c, precision.a, precision.b, precision.c)
...
temp1 <- data.frame(surface="a", method=c("bart_basic", "linear_reg", "ps_match", "ipw") , temp_reshape(results.a, precision.a) )

all_results <- rbind(all_results, temp1)


temp1 <- data.frame(surface="a", method=c("bcf", "bcf_att", "ps_bart", "bcf_late") , temp_reshape(results.a.2, precision.a.2) )
all_results <- rbind(all_results, temp1)

temp1 <- data.frame(surface="b", method=c("bart_basic", "linear_reg", "ps_match", "ipw") , temp_reshape(results.b, precision.b) )
all_results <- rbind(all_results, temp1)

temp1 <- data.frame(surface="b", method=c("bcf", "bcf_att", "ps_bart", "bcf_late") , temp_reshape(results.b.2, precision.b.2) )
all_results <- rbind(all_results, temp1)

temp1 <- data.frame(surface="c", method=c("bart_basic", "linear_reg", "ps_match", "ipw") , temp_reshape(results.c, precision.c) )
all_results <- rbind(all_results, temp1)

temp1 <- data.frame(surface="c", method=c("bcf", "bcf_att", "ps_bart", "bcf_late") , temp_reshape(results.c.2, precision.c.2) )
all_results <- rbind(all_results, temp1)
